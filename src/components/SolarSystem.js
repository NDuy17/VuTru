...
const SolarSystem = ({ planets, onPlanetPress, onMoonPress }) => {
  ...
  useEffect(() => {
    // 🔥 Optimized: Use a fixed interval (30 FPS) instead of Animated.loop for better performance on weak devices
    const interval = setInterval(() => {
      rotRef.current.time += 1000 / 30;
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        if (touches.length === 2) {
          ...
          rotRef.current.y += gestureState.dx * 0.001;
          rotRef.current.x += gestureState.dy * 0.001;
        }
  ...
  const planetData = useMemo(() => {
    ...
    return planets.map((p) => {
      ...
      if (!p.isSun) {
        ...
        const orbitRadius = 300 + 560 * Math.pow(distanceAU, 0.45);
  ...
  const moonData = planetData
    ...
    .flatMap((planet) =>
      planet.moonsInfo.map((moon, index) => {
        ...
        const rawSize = getMoonVisualSize(moon.diameter, planet.diameter, planet.size);
        return {
          ...moon,
          planet,
